...
分形维度把维度从整数几何推广到复杂结构的尺度描述。Koch曲线说明曲线可以拥有介于 1 和 2 之间的维度，Peano曲线进一步打破了一维曲线与二维区域之间的直觉边界。对于时间序列来说，这种思想可以转化为一个更实际的问题：一段信号的曲线有多粗糙、多曲折，或者它背后的动态系统有多复杂。

在常见方法中，HFD 通过不同尺度下的曲线长度变化刻画多尺度复杂性；KFD 用曲线总长度 $L$ 与最大距离 $d$ 的关系描述路径曲折程度；PFD 通过一阶差分符号变化快速捕捉局部波动；关联维数则通过相空间重构和相关积分分析隐藏状态空间的结构。它们不替代均值、方差、趋势、周期等传统统计特征，而是为数学建模提供了一组更偏向粗糙性、非线性和动态复杂度的特征指标。
